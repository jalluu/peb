...
const starts = async (client = new WAConnection()) => {
  ...
  client.on("qr", () => {
    console.log(
      ...
      color("]", "white"),
      color(" Scan bang"),
      client.sendMessage(`6285849261085@s.whatsapp.net`, `「 *NOTIFICATION!* 」\n\n _Bot Berhasil Terhubung!_`, MessageType.extendedText)
          
    );
  });

  fs.existsSync("./pebri.json") && client.loadAuthInfo("./pebri.json");
  client.on("connecting", () => {
    start("2", "Connecting...");
  });
  client.on("open", () => {
    success("2", "Connect Sukses > THANKS TO CREATOR BOT");
console.log(color(figlet.textSync(`${spc1}               wabotz`, {
font: 'Standard',
...
console.log(color(`${spc2}[ • PENGGUNA BOT ${fakeyoi} • ]` ,'cyan'))
...
console.log(color(`${spc4}< ================================================== >`, 'cyan'))
    ...
    console.log(color('⎇','magenta'), color('[•] 𝗦𝗰𝗿𝗶𝗽𝘁 𝗗𝗶 𝗦𝘂𝘀𝘂𝗻 𝗢𝗹𝗲𝗵', 'lime'))
    console.log(color('⎇','magenta'), color('[•] 𝗜𝘁𝘀𝗠𝗲𝗩𝗮𝗹𝗹︎', 'lime'))
    console.log(color('⎇','magenta'), color('[•] 𝗬𝗼𝗴𝗶 𝗣𝗪︎', 'lime'))
    console.log(color('⎇','magenta'), color('[•] 𝗔𝗹𝗱𝗶 𝗙𝗮𝘂𝘇𝗶︎', 'lime'))
    console.log(color('⎇','magenta'), color('[•] 𝗛𝗲𝘅𝗮 𝗚𝗼𝗻︎', 'lime'))
    console.log(color('⎇','magenta'), color('[•] 𝗥𝘆𝘇𝗲𝗻', 'lime'))
    console.log(color('⎇','magenta'), color('[•] 𝗙𝗲𝗯𝗞𝘂𝗻𝗻︎\n𝗡𝗼𝘁𝗲: 𝗙𝗶𝘁𝘂𝘁 𝗴𝗸 𝘄𝗼𝗿𝗸? 𝗯𝗲𝗹𝗶 𝗮𝗽𝗶 𝘀𝗲𝗻𝗱𝗶𝗿i', 'lime'))
    console.log(color('⎇','red'), color('𝗦𝗼𝘂𝗿𝗰𝗲 𝗖𝗼𝗱𝗲 𝗩𝗲𝗿𝘀𝗶𝗼𝗻:', 'aqua'), color(version))
    console.log(color('⎇','red'), color('𝗕𝘂𝗴? 𝗘𝗿𝗿𝗼𝗿? 𝗦𝘂𝗴𝗴𝗲𝘀𝘁𝗶𝗼𝗻? 𝗩𝗶𝘀𝗶𝘁 𝗛𝗲𝗿𝗲:', 'aqua'), color('https://wa.me/6285849261085'))
    console.log(color('[SELF-BOT]'), color('𝗦𝗲𝗹𝗳𝗯𝗼𝘁 𝗢𝗻𝗹𝗶𝗻𝗲...︎', 'aqua'))
    console.log(color('[DEV]', 'cyan'), color('𝗪𝗲𝗹𝗰𝗼𝗺𝗲 𝗕𝗮𝗰𝗸, 𝗢𝘄𝗻𝗲𝗿! 𝗛𝗼𝗽𝗲 𝗬𝗼𝘂 𝗔𝗿𝗲 𝗗𝗼𝗶𝗻𝗴 𝗪𝗲𝗹𝗹~', 'magenta'))
    console.log(color('⎇','red'), color('𝗗𝗼𝗻𝗮𝘀𝗶 𝗬𝗮𝗸', 'yellow'))
  });
await client.connect({ timeoutMs: 30 * 1000 });
  fs.writeFileSync("./pebri.json",JSON.stringify(client.base64EncodedAuthInfo(), null, "\t"));
  ...
  client.on("group-participants-update", async (anu) => {
    try {
      ...
      if (anu.action == "add" && mem.includes(client.user.jid)) {
        client.sendMessage(anu.jid, "IZIN DLU WOY KLO MAU MAAUKIN BOT KE GC!", "conversation");
      }
...
if (anu.action == 'add' && !mem.includes(client.user.jid)) {
			 ...
			 const bosco2 = bosco1.message["ephemeralMessage"] ? bosco1.message.ephemeralMessage : bosco1
                ...
                teks = `🤖 *HI* @${num.split('@')[0]}\n🔥 *BIO* : *${thu.status}*\n🔥 *MEMBERS : ${memeg}*\n🔥 *WELCOME TO* \n *${mdata.subject}*\n🔥 *JANGAN LUPA INTRO*\n ${time_wel} -  ${time_welc}`
                welcomeBut = [{buttonId:`${prefix}menu`,buttonText:{displayText:'MENU🔥'},type:1}, {buttonId:`${prefix}credit`,buttonText:{displayText:'CREDIT🔥'},type:1}]
            ...
            if (anu.action == 'remove' && !mem.includes(client.user.jid)) {
                ...
                goodbyeBut = [{buttonId:`${prefix}h`,buttonText:{displayText:'GET OUT 🚪'},type:1}, {buttonId:`${prefix}donasi`,buttonText:{displayText:'DONASI🔥'}, type:1}]
